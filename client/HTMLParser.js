...
function computeCSS(elem){
    const elems = stack.slice().reverse(); // 获取父元素
    ...
    for(let rule of rules){
        ...
        for(let i = 0; i<elems.length; i++){
            if(match(elems[i], selectorParts[j])){
                j++;
            }
        }
        ...
        if(matched){
            ...
            const computedStyle = elem.computedStyle;
            ...
            console.log(elem.computedStyle);
        }
    }
}
